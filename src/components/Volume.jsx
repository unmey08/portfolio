...
const Volume = ({theme}) => {
  ...
  return (
    <div className={`hidden md:block fixed bottom-2 right-10 mt-1 w-10 h-10 cursor-pointer object-contain ${pathname === '/' ? 'text-black-500' : theme === 'dark' ? 'text-gray-200' : 'text-black-500'} font-bold`}>
        <button className={`w-8 h-8 leading-9 text-3xl rounded-full m-1 ${pathname === '/' ? 'text-black' : theme === 'dark' ? 'text-white' : 'text-black'}`} onClick={() => setIsAudioPlaying(!isAudioPlaying)}>
            {isAudioPlaying ? (
